...
# Add local libs folder (libs/ is one level up from Script/)
...
def run_script(script_path: str, output_path: str, debug_mode: bool=False):
	...
	def has_variable(line: str) -> int:
			"""Checks if a line contains a variable placeholder ${var} OR a CSV lookup <file>{index}."""
			if isinstance(line, str):
				# Matches ${any_var} OR <any_file>{any_digit}
				pattern = r"(\$\{[^}]+\})|(<[^>]+>\{(\d+)\})"
				match = re.search(pattern, line)
				return 1 if match else 0
			return 0
	def xxhas_variable(line: str) -> int:
	...
	def xxreplace_variables_with_current_values(line: str, variable_arrays: dict) -> str:
			...
			# 1. ALWAYS handle standard variables first (${var})
			# If no variables exist, this loop simply does nothing and moves on
			var_matches = re.findall(r"\$\{([^}]+)\}", line)
			for var_name in var_matches:
				if var_name in variable_arrays:
					current_value = variable_arrays[var_name].get('current_value', 'NULL')
			...
			# 2. ALWAYS handle CSV lookups next (<file>{index})
			# This captures indices like {81}, {81.0}, or even {${var}} (after step 1)
			# Pattern matches: < (filename) > { (digits or dots) }
			csv_matches = re.findall(r"<([^>]+)>\s*\{([\d\.]+)\}", line)
			...
			for file_name, index_str in csv_matches:
				...
				# Attempt to convert index_str (like "81.0") to a valid integer index
				try:
					index = int(float(index_str))
				except ValueError:
					# If it's still not a number (e.g. if the variable replacement failed)
					log_print(f" \033[33mWarning: Could not resolve index '{index_str}' to a number for {file_name}.\033[0m")
					continue

				replacement_value = "NULL" 

				# Cache check and file loading
				if file_path not in csv_cache:
					if os.path.exists(file_path):
						try:
							with open(file_path, mode='r', encoding='utf-8-sig') as f:
				...
				if file_data is not None:
					...
					# This handles cases where file was missing or invalid
					replacement_value = "NULL"

				# Final replacement for this specific CSV tag
				pattern = re.compile(rf"<{re.escape(file_name)}>\s*\{{{re.escape(index_str)}\}}")
				line = pattern.sub(str(replacement_value), line)
...
╚══██╔══╝██╔════╝██╔════╝╚══██╔══╝      ██╔════╝██║     ██╔═══██╗██║    ██║
   ...
   ╚═╝   ╚══════╝╚══════╝   ╚═╝         ╚═╝     ╚══════╝ ╚═════╝  ╚══╝╚══╝ 
		...
		log_print_panner("Copyright (c) 2025 TestFlow".center(80))
		...
		log_print_panner("-" * 80)
		log_print_panner("www.testflowinc.com".center(80))
		log_print_panner("-" * 80)
		log_print_panner("©TestFlow".center(80))
		log_print_panner("=" * 80)
   ...
   ╚═╝   ╚══════╝╚══════╝   ╚═╝          ╚════╝  ╚═════╝ ╚═╝  ╚═══╝╚══════╝
	...
	def safe_eval_bool(expr: str) -> bool:
		...
		def _eval(n):
			...
			# Numeric / constant literals (Python 3.8+)
			...
			# Compatibility for Python 3.7 and below
		...
		while(running_script):
			...
			if check_line_prefix(Current_line, "Loop_start"):
				...
				if check_line_prefix(command, "wait"):
					...
					Varible2save= var2save_name(extract_prefixed_line(command, "save2var "))
			...
			#	 show_message_dialog("Waiting you",pause_message)
				...
				if image_data is not None:
					...
					log_print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}]: Warning: No SET data received.")
					update_csv_cell(outpath,Data_line,action_column_title,"ERROR: NO DATA")				 
